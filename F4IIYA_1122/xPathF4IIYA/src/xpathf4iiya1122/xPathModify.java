...
public class xPathModify {
	...
	public static void main(String[] args) {
		try {
			File inputFile = new File("F4IIYA_orarend.xml");
			DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
			DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
			Document doc = dBuilder.parse(inputFile);
			doc.getDocumentElement().normalize();

			modifyElementValue(doc, "3", "oktato", "Kiss Virág");

			modifyElementValue(doc, "4", "oktato", "Tóth István");

			modifyElementValue(doc, "7", "targy", "Szoftvertechnológiák");
			...
			StreamResult result = new StreamResult(new File("F4IIYA_orarend1.xml"));
